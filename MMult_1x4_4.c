...
// 本文中的矩阵是列主序存储的，使用C语言内置的数组来表示矩阵，但是C语言的数组存储方式是行主序
// 所以需要用宏转换一下
...
void AddDot1x4(int k, double* a, int lda, double* b, int ldb, double* c, int ldc) {
  ...
  for (int p = 0; p < k; p += 4) {
    ...
    a_0p = A(0, p + 1);
    // 共享了对A的访问，重复利用A
    c_00 += a_0p * *(b_p0_ptr + 1);
    c_01 += a_0p * *(b_p1_ptr + 1);
    c_02 += a_0p * *(b_p2_ptr + 1);
    c_03 += a_0p * *(b_p3_ptr + 1);

    a_0p = A(0, p + 2);
    // 共享了对A的访问，重复利用A
    c_00 += a_0p * *(b_p0_ptr + 2);
    c_01 += a_0p * *(b_p1_ptr + 2);
    c_02 += a_0p * *(b_p2_ptr + 2);
    c_03 += a_0p * *(b_p3_ptr + 2);

    a_0p = A(0, p + 3);
    // 共享了对A的访问，重复利用A
    c_00 += a_0p * *(b_p0_ptr + 3);
    c_01 += a_0p * *(b_p1_ptr + 3);
    c_02 += a_0p * *(b_p2_ptr + 3);
    c_03 += a_0p * *(b_p3_ptr + 3);
